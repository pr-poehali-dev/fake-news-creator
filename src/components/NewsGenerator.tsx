...
function getMultipleRandomItems<T>(array: T[], count: number, seed: string): T[] {
  ...
  for (let i = 0; i < Math.min(count, 15); i++) { // Ограничиваем до 15 максимум
    let attempts = 0;
    let randomIndex: number;
    
    do {
      randomIndex = Math.abs(hashCode(`${seed}-attempt-${i}-${attempts++}`)) % array.length;
      if (attempts > 100) break; // Избегаем бесконечных циклов
    } while (usedIndices.has(randomIndex));
    
    usedIndices.add(randomIndex);
    result.push(array[randomIndex]);
    
    if (attempts > 100) break; // Дополнительная защита
  }
...
export const NewsGenerator = ({ date }: { date: string }) => {
  ...
  useEffect(() => {
    ...
    // Обернем в try-catch для отлова ошибок
    try {
      // Имитация загрузки данных с таймаутом
      const timer = setTimeout(() => {
        ...
        try {
          ...
          console.error("Ошибка при генерации новостей:", err);
          if (isMounted) {
            setError("Произошла ошибка при генерации новостей. Попробуйте другую дату.");
            setLoading(false);
          }
        }
      }, 800);
      ...
      console.error("Ошибка при настройке генерации:", err);
      if (isMounted) {
        setError("Произошла неожиданная ошибка. Попробуйте обновить страницу.");
        setLoading(false);
      }
    }
  }, [date]);
  ...
  const getEpochType = (year: number): 'ancient' | 'pre-industrial' | 'industrial' | 'modern' | 'near-future' | 'far-future' => {
    if (isNaN(year)) return 'modern'; // Защита от NaN
  ...
  const generateNews = (dateString: string): NewsItem[] => {
    try {
      ...
      if (!epochData) {
        console.error("Не удалось определить эпоху для генерации новостей");
        return [];
      }
      
      // Генерируем от 5 до 10 новостей (ограничиваем верхний предел для производительности)
      const seedBase = `${day}-${month}-${year}`;
      const newsCount = 5 + Math.abs(hashCode(seedBase) % 6);
      
      // Проверка на возможность получения уникальных заголовков
      const titleCount = Math.min(newsCount, epochData.titles.length);
      const descCount = Math.min(newsCount, epochData.descriptions.length);
      
      // Выбираем уникальные заголовки и описания
      const selectedTitles = getMultipleRandomItems(epochData.titles, titleCount, `${seedBase}-titles`);
      const selectedDescriptions = getMultipleRandomItems(epochData.descriptions, descCount, `${seedBase}-descs`);
      ...
      for (let i = 0; i < titleCount; i++) {
        ...
        generatedNews.push({
          id: i,
          title: selectedTitles[i] || "Новость дня",
          description: selectedDescriptions[i % selectedDescriptions.length] || "Подробности скоро появятся.",
      ...
      console.error("Ошибка в generateNews:", error);
  ...
  if (!date) {
    return (
      <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
        ...
        <p className="text-red-600">Дата не указана. Пожалуйста, введите дату.</p>
      </div>
    );
  }
  ...
  try {
        ...
        <div className="space-y-4">
          {news.length > 0 ? (
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <CardDescription className="flex items-center mt-2">
                        ...
                        <span className="bg-accent text-accent-foreground text-xs px-2 py-1 rounded-full mr-2">{item.category}</span>
            ...
            <div className="text-center p-10 bg-muted/20 rounded-lg">
              <p>Новости не найдены. Попробуйте другую дату.</p>
            </div>
          )}
        </div>
      </div>
    ...
    console.error("Ошибка при рендеринге новостей:", err);
    return (
      <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
        <div className="flex items-center text-red-700 mb-2">
          <AlertCircle className="h-5 w-5 mr-2" />
        ...
        <p className="text-red-600">Произошла ошибка при отображении новостей. Попробуйте другую дату.</p>
      </div>
    );
  }
};
